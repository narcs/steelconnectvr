  ...
  void Update() {
    ...
    if (IsReady) {
      ...
      if (transform.hasChanged) {
        // NOTE(andrew): Show() has the side effect of recalculating the position of the keyboard, so this
        // normally updates its position if the player moves. However, this also makes the keyboard show
        // if the player moves, which happens every time we drag the globe (which actually moves the player).
        // So this Show() call has been disabled.
        //Show();
